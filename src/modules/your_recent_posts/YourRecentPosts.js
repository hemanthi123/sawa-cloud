...
db.collection('users').get().then((snippetsSnapshot) =>{
  ...
  //db.collection('/users/'+doc.id+'/post').where("approved_status", "==", 0).get().then((querySnapshot) => {
    db.collection('/users/'+doc.id+'/post').where("approved_status", "!=", -2).get().then((querySnapshot) => {
...
<div className="home-su-welcome1"><h4>Your Recent posts</h4> </div>
...
<div className="second_col_parent_my_post">
...
export default YourRecentPosts;